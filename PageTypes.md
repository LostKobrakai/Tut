...
A last advantage of a custom class is the ability to adhere to any interface you might be using in your application. Say you've a third party library, which does accept an interface as input. You can now go ahead and implement that interface for a specific page type and after that simply pass these pages into the library. Same goes for our custom repository.
